...
1. Load resume
2. Fetch jobs  ← now supports --source mock | jobspy | jsearch | llm_careers
...
Run:
  ...
  python main.py --source jsearch
  python main.py --source llm_careers
...
def run(
    ...
    jobs = fetch_jobs(
        ...
        location=location,
        source=source,          # ← NEW
        resume_text=resume_text,  # ← needed by llm_careers
        max_results=max_results,  # ← NEW
    )
    ...
    for i, job in enumerate(selected_jobs, 1):
        ...
        if (folder / "improved_resume.txt").exists():
            print("  ⏭ Already processed — skipping. Delete the folder to rerun.")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        default="mock",
        choices=["mock", "jobspy", "jsearch", "llm_careers"],
        help=(
            "Job data source (default: mock):\n"
            "  mock        — hardcoded sample jobs, no internet needed\n"
            "  jobspy      — scrapes Indeed/LinkedIn/Glassdoor (free, no key)\n"
            "  jsearch     — RapidAPI aggregator (set RAPIDAPI_KEY in .env)\n"
            "  llm_careers — AI picks companies from your resume, scrapes their pages"
        ),
    )
    parser.add_argument(
